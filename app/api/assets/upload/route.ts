...
export async function POST(request: Request) {
  ...
  const maxSize = isVercelRuntime ? 3 * 1024 * 1024 : 12 * 1024 * 1024;
  if (file.size > maxSize) {
    return NextResponse.json(
      { error: `Screenshot must be smaller than ${Math.round(maxSize / 1024 / 1024)}MB.` },
      { status: 400 },
    );
  }
  ...
  if (isVercelRuntime) {
    return NextResponse.json({
      asset: {
        id,
        type: "screenshot",
        src: `data:${file.type};base64,${bytes.toString("base64")}`,
